...
export function DashboardSidebar() {
    ...
    const activeUsername = workspace?.username || (user?.email ? user.email.split("@")[0] : "rajnish");
    const activeDisplayName = isImpersonating
        ? workspace?.display_name || "Creator Workspace"
        : user?.name || workspace?.display_name || "Creator";
    const activeEmail = isImpersonating ? `${activeUsername}@outsyra.com` : user?.email || "creator@outsyra.com";
    const activeAvatar = isImpersonating
        ? workspace?.avatar_url || user?.avatar
        : user?.avatar || workspace?.avatar_url || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=80&auto=format&fit=crop&q=80";
        ...
        <aside className="w-64 flex-shrink-0 border-r border-zinc-200 dark:border-white/[0.08] bg-white dark:bg-[#08090d] flex flex-col h-screen sticky top-0 transition-colors duration-200">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-1.5">
                            <p className="text-[10px] text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-wider">
                                {isSuperAdmin ? "Tenant Hub" : "Creator Studio"}
                            </p>
                            {isSuperAdmin && (
                                <span className="bg-amber-500/15 text-amber-600 dark:text-amber-400 text-[8px] font-extrabold px-1 py-0.2 rounded border border-amber-500/30 uppercase">
                                    Admin
                                </span>
                            )}
                        </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-3 space-y-5">
                {navGroups.map((group, idx) => {
                    // Filter items: only show Admin & System Health to superadmin/admin
            ...
            <div className="p-3 border-t border-zinc-200 dark:border-white/[0.08] space-y-2.5 bg-zinc-50 dark:bg-[#0f1117]/60">
                ...
                >
                    <span className="flex items-center gap-1.5 truncate">
                        ...
                        <span className="truncate">outsyra.com/{activeUsername}</span>
                    </span>
                ...
                <div className="flex items-center justify-between gap-2 px-1">
                    <div className="flex items-center gap-2.5 min-w-0">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <p className="text-[10px] text-zinc-500 truncate">{activeEmail}</p>
                        </div>
                    </div>
                    <button
                        onClick={logout}

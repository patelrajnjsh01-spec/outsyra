            ...
            <main className="p-6 md:p-8 space-y-8 max-w-7xl">
                <div className="glass-card p-6 rounded-3xl border border-zinc-200 dark:border-white/10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6 shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="h-12 w-12 rounded-2xl bg-pink-500/10 border border-pink-500/20 flex items-center justify-center text-pink-600 dark:text-pink-400">
                            <Instagram className="h-6 w-6" />
                        </div>
                        <div>
                            <div className="flex items-center gap-2">
                                <h3 className="text-base font-bold text-zinc-900 dark:text-white">Official Meta Graph API Connection</h3>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-2 space-y-4">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-sm font-semibold text-zinc-900 dark:text-white">Active Automation Rules</h3>
                        ...
                        <div className="space-y-4">
                            {rules.map((rule) => (
                                <Card key={rule.id} className="glass-card border-zinc-200 dark:border-white/10 p-6 space-y-4 shadow-sm">
                                    <div className="flex items-center justify-between">
                                        <div className="flex items-center gap-2">
                                            <span
                                                className={`h-2.5 w-2.5 rounded-full ${
                                                    rule.is_active ? "bg-emerald-500" : "bg-zinc-400 dark:bg-zinc-600"
                                                }`}
                                            />
                                            <h4 className="text-sm font-bold text-zinc-900 dark:text-white">{rule.name}</h4>
                                        </div>
                                        <div className="flex items-center gap-2">
                                    ...
                                    <div className="p-4 rounded-2xl bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200 dark:border-white/5 space-y-2 text-xs">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <div className="flex gap-1.5 flex-wrap">
                                                {rule.trigger_keywords?.map((kw, i) => (
                                                    <span
                                                        key={i}
                                                        className="px-2 py-0.5 rounded-md bg-indigo-500/10 dark:bg-indigo-500/20 text-indigo-600 dark:text-indigo-300 font-mono text-[10px] font-bold"
                                                    >
                                                        "{kw}"
                                                    </span>
                                                ))}
                                        ...
                                        <div className="pt-2 border-t border-zinc-200/60 dark:border-white/5">
                                            <span className="text-zinc-600 dark:text-zinc-400 font-medium">Automated Response:</span>
                                            <p className="text-zinc-800 dark:text-zinc-200 mt-1 font-mono text-[11px] leading-relaxed bg-white dark:bg-zinc-950/60 p-2.5 rounded-xl border border-zinc-200 dark:border-white/5">
                    ...
                    <div className="space-y-4">
                        <h3 className="text-sm font-semibold text-zinc-900 dark:text-white flex items-center gap-1.5">
                            ...
                            <span>Realtime Webhook Logs</span>
                        </h3>
                        <Card className="glass-card border-zinc-200 dark:border-white/10 p-4 space-y-3 shadow-sm">
                            {[
                                ...
                                <div key={i} className="p-3 rounded-xl bg-zinc-50 dark:bg-zinc-900/40 border border-zinc-200 dark:border-white/5 text-xs space-y-1">
                                    <div className="flex items-center justify-between">
                                        <span className="font-semibold text-zinc-900 dark:text-white">{log.user}</span>
                                        <span className="text-[10px] text-zinc-500">{log.time}</span>
                                    </div>
                                    <div className="flex items-center justify-between text-[11px]">
                                        <span className="text-indigo-600 dark:text-indigo-400 font-mono">Comment: "{log.kw}"</span>
            ...
            {createModal && (
                <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="glass-card p-6 md:p-8 rounded-3xl border border-zinc-200 dark:border-white/10 max-w-lg w-full space-y-5 animate-in fade-in zoom-in-95 shadow-2xl">
                        <h3 className="text-lg font-bold text-zinc-900 dark:text-white">Create Instagram Keyword Trigger</h3>
                        <form onSubmit={handleCreateRule} className="space-y-4">
                            <div>
                                <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1.5">Rule Name</label>
                                <Input
                                    required
                            ...
                            <div>
                                ...
                                <textarea
                                    rows={4}
                                    required
                                    className="w-full rounded-xl border border-zinc-200 dark:border-white/10 bg-white/80 dark:bg-zinc-900/60 p-3 text-xs text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:ring-2 focus:ring-indigo-500 focus:outline-none"

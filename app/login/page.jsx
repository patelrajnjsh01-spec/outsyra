  ...
  async function handleLogin(event) {
    ...
    try {
      ...
      if (
        ...
        setError("Supabase backend is currently in preview mode. Click 'Explore Demo Workspace' below to test immediately!");
      } else {
  ...
  async function handleGoogleLogin() {
    ...
    try {
      ...
      if (!url || url.includes("your-project-id")) {
        // Instant demo access when Supabase is in preview
  ...
  return (
    <main className="grid-bg flex min-h-screen items-center justify-center px-4 py-12">
      <section className="w-full max-w-md">
        ...
        <div className="glass-panel rounded-2xl border p-6 shadow-xl sm:p-8 space-y-5">
          {error && (
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full gap-3 border-white/10 bg-white/5 hover:bg-white/10 text-white font-medium h-11"
          ...
          <div className="relative">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-zinc-900 px-2 text-muted-foreground">Or with Email</span>
            </div>
          </div>
          ...
          <form className="space-y-4" onSubmit={handleLogin}>
            <label className="block text-sm font-medium">
              ...
              <Input
                ...
                required
                autoComplete="email"
                placeholder="creator@outsyra.com"
              />
            </label>
            <label className="block text-sm font-medium">
              Password
          ...
          <div className="relative pt-2">
            ...
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-zinc-900 px-2 text-muted-foreground">Demo Mode</span>
            </div>
          </div>
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full border-indigo-500/30 bg-indigo-500/10 text-indigo-300 hover:bg-indigo-500/20"

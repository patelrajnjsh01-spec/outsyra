...
import { Plus, CheckCircle2, Video, Sparkles, Trash2 } from "lucide-react";
        ...
        <div className="flex-1 flex flex-col bg-background text-foreground transition-colors duration-200">
            ...
            <main className="p-6 md:p-8 space-y-6 max-w-7xl">
                ...
                <div className="glass-card p-6 rounded-3xl border border-indigo-500/20 bg-gradient-to-r from-indigo-500/5 via-purple-500/5 to-transparent dark:from-indigo-950/40 dark:via-purple-950/20 dark:to-zinc-950 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 shadow-sm">
                    <div>
                        <h3 className="text-sm font-bold text-zinc-900 dark:text-white">Start a Live Client Coaching Session</h3>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <Button
                            variant="outline"
                            size="sm"
                            className="gap-2 text-xs border-indigo-500/30 bg-indigo-500/10 text-indigo-600 dark:text-indigo-200 hover:bg-indigo-500/20"
                ...
                <div className="flex justify-between items-center">
                    <div>
                        <h3 className="text-base font-bold text-zinc-900 dark:text-white">Active Coaching Offers</h3>
                        <p className="text-xs text-zinc-600 dark:text-zinc-400">Clients can book directly through your link-in-bio storefront.</p>
                    </div>
                    <Button variant="gradient" onClick={() => setCreateModal(true)} className="gap-2 text-xs">
                        <Plus className="h-4 w-4" />
                        <span>Create Coaching Offer</span>
                    </Button>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {offers.map((offer) => (
                        <Card key={offer.id} className="glass-card border-zinc-200 dark:border-white/10 p-6 space-y-5 flex flex-col justify-between shadow-sm">
                            <div className="space-y-4">
                                ...
                                <div>
                                    <h4 className="text-lg font-bold text-zinc-900 dark:text-white">{offer.title}</h4>
                                    <p className="text-xs text-zinc-600 dark:text-zinc-400 mt-1 leading-relaxed">{offer.description}</p>
                                </div>
                            ...
                            <div className="pt-4 border-t border-zinc-200/60 dark:border-white/5 space-y-3">
                                <div className="flex items-center justify-between text-xs text-zinc-500">
                                    ...
                                    <button
                                        ...
                                        className="text-zinc-400 hover:text-red-500 text-xs flex items-center gap-1 cursor-pointer"
                                    >
                                ...
                                <div className="grid grid-cols-2 gap-2">
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        className="w-full gap-1.5 text-xs border-emerald-500/30 bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 hover:bg-emerald-500/20"
                                        onClick={() => startInstantCall("google_meet")}
                                    >
                                    ...
                                    <Button
                                        variant="outline"
                                        size="sm"
                                        className="w-full gap-1.5 text-xs border-indigo-500/30 bg-indigo-500/10 text-indigo-600 dark:text-indigo-300 hover:bg-indigo-500/20"
                                        onClick={() => startInstantCall("jitsi")}
                                    >
                                        <Sparkles className="h-3.5 w-3.5" />
                                        <span>Launch Jitsi</span>
                                    </Button>
                                </div>
                            </div>
                        </Card>
            ...
            {createModal && (
                <div className="fixed inset-0 bg-black/60 dark:bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="glass-card p-6 md:p-8 rounded-3xl border border-zinc-200 dark:border-white/10 max-w-md w-full space-y-4 animate-in fade-in zoom-in-95 shadow-2xl">
                        <h3 className="text-base font-bold text-zinc-900 dark:text-white">Create Coaching Program</h3>
                        <form onSubmit={handleCreateOffer} className="space-y-4">
                            <div>
                                <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Program Title</label>
                                <Input
                                    required
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-medium text-zinc-700 dark:text-zinc-300 mb-1">Duration (Mins)</label>
                                    <Input
                                        type="number"
                                        required
                                        placeholder="60"

...
export async function getWorkspace(username = "rajnish") {
    ...
    if (supabase) {
        try {
            ...
            if (data && !error) {
                cacheWorkspace = data;
                if (typeof window !== "undefined") {
                    localStorage.setItem(`outsyra_ws_${username}`, JSON.stringify(data));
                }
                return data;
            }
        } catch (err) {
    ...
    if (typeof window !== "undefined") {
        try {
            const res = await fetch(`/api/workspace/sync?username=${encodeURIComponent(username)}`);
            if (res.ok) {
                const data = await res.json();
                if (data.workspace) {
                    cacheWorkspace = data.workspace;
                    localStorage.setItem(`outsyra_ws_${username}`, JSON.stringify(data.workspace));
        ...
        // 3. Try LocalStorage
        try {
            const saved = localStorage.getItem(`outsyra_ws_${username}`);
            if (saved) {
